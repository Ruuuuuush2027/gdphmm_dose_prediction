    ...
    def __getitem__(self, index):
        ...
        if self.phase == "train":
            if "with_aug" in self.cfig.keys() and not self.cfig["with_aug"]:
                self.aug = tt_augmentation(
                    KEYS, self.cfig["in_size"], self.cfig["out_size"], isocenter
                )
            else:
                self.aug = tr_augmentation(
                    KEYS, self.cfig["in_size"], self.cfig["out_size"], isocenter
                )
        ...
        comb_oar, cat_oar = combine_oar(
            In_dict, need_list, norm_oar=True, OAR_DICT=OAR_DICT
        )
        ...
        for key in self.scale_dose_Dict[PatientID].keys():
            if key in ["PTV_High", "PTV_Mid", "PTV_Low"]:
                opt_dose_dict[self.scale_dose_Dict[PatientID][key]["OPTName"]] = (
                    self.scale_dose_Dict[PatientID][key]["PDose"]
                    / self.cfig["dose_div_factor"]
                )
                if key != "PTV_High":
                    dose_dict[self.scale_dose_Dict[PatientID][key]["StructName"]] = (
                        self.scale_dose_Dict[PatientID][key]["PDose"]
                        / self.cfig["dose_div_factor"]
                    )
                else:
                    dose_dict[self.scale_dose_Dict[PatientID][key]["OPTName"]] = (
                        self.scale_dose_Dict[PatientID][key]["PDose"]
                        / self.cfig["dose_div_factor"]
                    )
        ...
        data_dict["data"] = torch.cat(
            (
                ...
                In_dict["Body"],
                In_dict["img"],
                data_dict["beam_plate"],
                data_dict["angle_plate"],
            ),
            axis=0,
        )  # , In_dict['obj_2DGy'], In_dict['obj_2DWei']
        ...
        data_dict["site"] = torch.tensor(self.site_list[index])
